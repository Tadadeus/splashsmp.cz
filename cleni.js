...
const cleni = [
  ...
  { section: "hrac", name: "Alexx_CZ", lore: true, role: "Hráč", roleClass: "role--gray", desc: "Bývalý prezident SplashSMP", quote: "Nic špatného jsem neudělal.", longDesc: "Alex (dříve známý jako King) je bývalý prezident SplashSMP. Mnoho hráčů jeho vládu neuznává a označuje ji za špatnou, on a jeho vláda ale umí tato tvrzení vyargumentovat tak, že nikdo nemá slov – právě proto, že na serveru neudělal nic špatného. Měl totiž 90 % mandátu dovolenou. Poslední zmínka o něm byla nalezena ve spisech šíleného doktora Luďka. Od té doby je nezvěstný.", link: "https://www.tiktok.com/@splashsmp_/video/7519070407203310870" },
  ...
  { section: "hrac", name: "LukeBot345", lore: true, role: "Hráč", roleClass: "role--gray", desc: "Bývalý prezident season 3", quote: "Kandidovat už nesmím.", longDesc: "Lukebot je bývalý prezident 3. sezóny. Jeho vláda byla neschopná a nekompetentní až tak, že mu bylo zakázáno dále kandidovat.", link: "https://www.tiktok.com/@splashsmp_/video/7510865755861273878" },
...
if (root) {
  ...
  root.innerHTML = sections.map(sec => {
    ...
    //   0) Ardaros / SweepingAttack (first: true) – vždy první
    ...
    if (sec.key === "hrac") {
      ...
      members.forEach((x, i) => { x._i = i; });   // pro stabilitu
      members.sort((a, b) => {
        const ra = rank(a.p), rb = rank(b.p);
        if (ra !== rb) return ra - rb;
        // V rámci skupiny s videem seskup stejné odkazy k sobě.
        if (ra === 1 && a.p.link !== b.p.link) {
          return a.p.link < b.p.link ? -1 : 1;
        }
        return a._i - b._i;   // jinak zachovej původní pořadí
      });
    }
