...
if (root) {
  ...
  // Tlačítko na událost v Historii – jen pokud má hráč "hist".
  function histLinkHTML(p) {
    if (!p.hist) return "";
    return `
      <a href="historie.html#${p.hist}" class="player-modal__link player-modal__link--hist">
        <i class="fas fa-clock-rotate-left"></i> Najdi ho v Historii
      </a>`;
  }
  ...
  function openPlayer(i) {
    const p = cleni[i];
    if (!p || !modal) return;
    const links = videoLinkHTML(p) + histLinkHTML(p);
    mBody.innerHTML = `
